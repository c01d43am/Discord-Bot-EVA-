...
if (context.params.event.content.startsWith('-kiss')) {
  const author = context.params.event.mentions.length
    ? context.params.event.mentions[0]
    : context.params.event.author;
  const {data} = await axios('https://api.waifu.pics/sfw/kiss');

  await lib.discord.channels['@0.1.2'].messages.create({
    channel_id: context.params.event.channel_id,
    content:
      author.id === context.params.event.author.id
        ? `<@${author.id}> XOXO:pleading_face: `
        : `<@${context.params.event.author.id}>want kiss<@${author.id}>`,
    embed: {
      image: {url: data.url},
      color: 0xffc0cb,
    },
