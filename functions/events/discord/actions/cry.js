...
if (context.params.event.content.startsWith('-cry')) {
  const author = context.params.event.mentions.length
    ? context.params.event.mentions[0]
    : context.params.event.author;
  const {data} = await axios('https://api.waifu.pics/sfw/cry');

  await lib.discord.channels['@0.1.2'].messages.create({
    channel_id: context.params.event.channel_id,
    content:
      author.id === context.params.event.author.id
        ? `<@${author.id}> Ohhh dear, who made you cry love?`
        : `<@${context.params.event.author.id}> It feels good when you have a shoulder to cry on  <@${author.id}>cry`,
    embed: {
      image: {url: data.url},
      color: 0xffc0cb,
    },
